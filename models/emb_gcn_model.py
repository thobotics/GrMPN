...
class EncodeKinematic(snt.AbstractModule):
    ...
    def __init__(self, name="EncodeKinematic", placeholders=None):
        super(EncodeKinematic, self).__init__(name=name)

        self.placeholders = placeholders
        self.gcn_encoder = GraphConvolution(input_dim=1,
    ...
    def _build(self, input_op, input_hir_op, num_propagate=1):
        gcn_encoded = self.gcn_encoder(input_hir_op)
        gcn_encoded = tf.cast(gcn_encoded, tf.float64)
        global_encoded = tf.unsorted_segment_sum(
            data=gcn_encoded,
            segment_ids=self.placeholders["segment_ph"],
            num_segments=tf.shape(input_op.nodes)[0]
        )
